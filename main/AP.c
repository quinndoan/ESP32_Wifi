...
//     char ssid[32] = {0}; // Bộ đệm cho SSID
//     char password[64] = {0}; // Bộ đệm cho Password

//     // Phân tích dữ liệu POST nhận được
//     int num_scanned = sscanf(buf, "mode=%d&ssid=%31[^&]&password=%63s", &mode, ssid, password);
...
//     // Kiểm tra nếu chế độ là STA Mode (mode = 0) và đã nhận được SSID và Password
//     if (mode == 0 && num_scanned == 3) {
//         ESP_LOGI(TAG, "STA Mode selected. SSID: %s, Password: %s", ssid, password);

//         // Lưu SSID và Password vào NVS
//         err = nvs_set_str(nvs_handle, "ssid", ssid);
//         if (err != ESP_OK) {
//             ESP_LOGE(TAG, "Failed to set SSID in NVS! Error: %s", esp_err_to_name(err));
...
//         if (err != ESP_OK) {
//             ESP_LOGE(TAG, "Failed to set Password in NVS! Error: %s", esp_err_to_name(err));
//             nvs_close(nvs_handle);
...
//         if (err != ESP_OK) {
//             ESP_LOGE(TAG, "Failed to commit changes in NVS! Error: %s", esp_err_to_name(err));
//             nvs_close(nvs_handle);
